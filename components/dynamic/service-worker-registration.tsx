...
const ServiceWorkerRegistration: React.FC<ServiceWorkerRegistrationProps> = ({
    ...
    const registerServiceWorker = async () => {
      try {
        ...
        if (reg.active && assetsToCache.length > 0) {
          ...
          setTimeout(() => {
            console.log('Sending asset list to service worker for caching:', assetsToCache.length);
  ...
  useEffect(() => {
    if (registration?.active && assetsToCache.length > 0) {
      ...
      const uniqueAssets = [...new Set(assetsToCache.filter(url => url && url.length > 0))];
      
      if (uniqueAssets.length > 0) {
        console.log('Sending updated asset list to service worker for caching:', uniqueAssets.length);
