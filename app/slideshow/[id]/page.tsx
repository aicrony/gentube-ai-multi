...
export default function SlideshowPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (loading) {
    return (
      <div
        className={`flex items-center justify-center min-h-screen ${theme === 'dark' ? 'bg-gray-900' : 'bg-gray-100'}`}
      >
        <div
  ...
  if (error) {
    return (
      <div
        className={`flex flex-col items-center justify-center min-h-screen ${theme === 'dark' ? 'bg-gray-900' : 'bg-gray-100'}`}
      >
  ...
  return (
    <div
      className={`min-h-screen flex flex-col ${theme === 'dark' ? 'bg-gray-900' : 'bg-gray-100'}`}
    >
      ...
      {isModalOpen && assets.length > 0 && currentAsset && (
        <Modal
          ...
          hasNext={
            ...
            originalSettings.current?.infiniteLoop ||
            false
          }
          hasPrevious={
            currentIndex > 0 || originalSettings.current?.infiniteLoop || false
          }
          ...
          slideshowInfiniteLoop={
            originalSettings.current?.infiniteLoop || false
          }
