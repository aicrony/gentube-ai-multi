    ...
    caches.keys().then((cacheNames) => {
      return Promise.all(
        cacheNames.filter((cacheName) => {
          ...
          return (cacheName.startsWith('slideshow-cache-') && cacheName !== CACHE_NAME) ||
                 (cacheName.startsWith('slideshow-images-') && cacheName !== IMAGE_CACHE_NAME) ||
                 (cacheName.startsWith('slideshow-assets-') && cacheName !== ASSET_CACHE_NAME);
        }).map((cacheName) => {
      ...
      caches.open(IMAGE_CACHE_NAME)
        .then((cache) => {
          ...
          const cacheNextBatch = (batchIndex) => {
            ...
            const endIndex = Math.min(startIndex + batchSize, assetsToCache.length);
            const batchPromises = [];
            
            for (let i = startIndex; i < endIndex; i++) {
              const assetUrl = assetsToCache[i];
              if (assetUrl && (isImageRequest(assetUrl) || isVideoRequest(assetUrl))) {
                console.log(`Caching asset ${i + 1}/${assetsToCache.length}: ${assetUrl}`);
