  ...
  try {
    ...
    for (const entity of entities) {
      ...
      if (
        ...
        console.log(`Found matching entity by scanning: Key ID ${keyId} matches ${assetId}`);
...
export async function addAssetToGallery(
  ...
  try {
    console.log(`Attempting to add asset ${assetId} to gallery for user ${userId}`);
...
export async function removeAssetFromGallery(
  ...
  try {
    console.log(`Attempting to remove asset ${assetId} from gallery for user ${userId}`);
    ...
    if (asset.SubscriptionTier !== 3) {
      console.log('Asset is not in the gallery (SubscriptionTier !=3):', asset.SubscriptionTier);
