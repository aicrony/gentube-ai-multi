...
export const decryptToken = (encryptedText: string, secretKey: string): string => {
  try {
  ...
  try {
    ...
    const entity = {
      ...
      data: {
        ...
        createdAt: new Date(),
        updatedAt: new Date(),
        expiresAt: tokenData.expiresAt ? new Date(tokenData.expiresAt * 1000) : undefined
      }
    };
...
export const getSocialToken = async (
  ...
  try {
    ...
    console.error(`Error retrieving ${platform} token for user ${userId}:`, error);
    return null;
  }
};
