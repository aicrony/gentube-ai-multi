 ...
 */
export async function getUserCreator(userId: string): Promise<string | null> {
  try {
    ...
    if (userCredits.length > 0) {
      if (userCredits[0].CreatorName) {
        console.log(
          `Found CreatorName via query for userId=${userId}: ${userCredits[0].CreatorName}`
        );
        return userCredits[0].CreatorName;
      } else if (userCredits[0].Name) {
        console.log(
          `Found Name via query for userId=${userId}: ${userCredits[0].Name}`
        );
